...
### Security
- **Script-safety patterns now cover FortiOS command-prefix abbreviations** (`ex`/`exe`/`execu…` for `execute`, `conf sys`/`conf rout` for the config stanzas), which previously bypassed the strict-mode denylist. Tcl script types (`tcl`, `tclgrp`) are refused under `FMG_SCRIPT_SAFETY=strict` at create, update, and execute time — their commands can be assembled at runtime, so static content screening cannot vouch for them. The denylist remains defense-in-depth; a least-privilege FMG API account is the primary control.
