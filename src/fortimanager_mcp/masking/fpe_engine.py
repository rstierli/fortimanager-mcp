"""Format-preserving encryption engine for reversible masking (issue #34).
...
#: Public alias: token grammars are derived from the cipher alphabet.
STR_ALPHABET = _STR_ALPHABET
...
MASKING_KEY_ENV = "FMG_MASKING_KEY"
...
#
# The "faz-mcp-fpe:v1" namespace is deliberately NOT renamed for the
# FortiManager port. It identifies the shared token protocol across the
# sibling MCPs: under one key, a value masks to the same ciphertext on
# both servers, so an analyst can correlate what FortiAnalyzer showed
# with what FortiManager shows. Renaming it would silently fork the
# format for zero benefit.
_TWEAK_LABELS = {
    ...
    # FortiManager-only. Serials are sealed with the url-tail construction
    # (base32 shield + string cipher) but under their own label, so serial
    # and URL ciphertext domains stay separate.
...
class FPEEngine:
    ...
    def from_env(cls, mask_suffix: str = DEFAULT_MASK_SUFFIX) -> "FPEEngine":
        """Build an engine from the ``FMG_MASKING_KEY`` environment variable.
    ...
    # ------------------------------------------------------------------ #
    # FortiManager envelopes                                             #
    # ------------------------------------------------------------------ #
    #
    # FortiAnalyzer emits bare, format-preserving IP and MAC tokens: a
    # masked address is itself a valid address carrying no marker (the
    # "IP wrinkle" above). On FortiManager that is unsafe in a way it is
    # not on a read-only log surface, because a tool argument becomes
    # estate configuration. FF3 is a permutation with no integrity tag,
    # so every address-shaped value decrypts to some valid address, and a
    # stale, rotated or foreign token would restore to a plausible wrong
    # address and be written. Wrapping these types in an explicit,
    # key-id-carrying envelope makes every token this server emits
    # recognizable, which is what lets the input guard refuse them.
    #
    # The ciphertext inside the envelope is unchanged, so it still equals
    # what FortiAnalyzer produces for the same value and key. Only the
    # presentation differs; adopting the envelopes on the sibling is
    # proposed on RFC #40.

    def mask_ip_token(self, value: str) -> str:
        """Mask an IP into a marked ``ip4-<kid>-<ct>`` / ``ip6-`` token."""
        kind = "ip6" if self._parse_ip(value).version == 6 else "ip4"
        return f"{kind}-{self._key_id}-{self.mask_ip(value)}"

    def unmask_ip_token(self, token: str) -> str:
        """Reverse :meth:`mask_ip_token`, verifying kind and key id.

        Engine-only: nothing on the tool boundary calls this, because v1
        never restores a token supplied as an argument.
        """
        kind, kid, ct = self._split_envelope(token, ("ip4", "ip6"))
        self._check_key_id(kid, token)
        plain = self.unmask_ip(ct)
        expected = 6 if kind == "ip6" else 4
        if self._parse_ip(plain).version != expected:
            raise MaskingError("token address family does not match its marker")
        return plain

    def mask_mac_token(self, value: str) -> str:
        """Mask a MAC into a marked ``mac-<kid>-<ct>`` token."""
        return f"mac-{self._key_id}-{self.mask_mac(value)}"

    def unmask_mac_token(self, token: str) -> str:
        """Reverse :meth:`mask_mac_token`, verifying the key id."""
        _, kid, ct = self._split_envelope(token, ("mac",))
        self._check_key_id(kid, token)
        return self.unmask_mac(ct)

    def seal_serial(self, value: str) -> str:
        """Seal a device serial into a ``sn-<kid>-<ct>`` token.

        Serials are uppercase alphanumerics, which the lowercase string
        alphabet cannot round-trip, so they take the url-tail treatment:
        base32-shield the raw bytes, then run the string cipher. Case
        survives exactly. Residual, inherited from that construction: the
        token length reveals the serial's length.
        """
        if not value.strip():
    ...
    def unseal_serial(self, token: str) -> str:
        """Reverse :meth:`seal_serial`, returning the exact original serial."""
        _, kid, ct = self._split_envelope(token, ("sn",))
        self._check_key_id(kid, token)
        encoded = self._decrypt_str("serial", ct)
        try:
            return base64.b32decode(encoded.upper() + "=" * (-len(encoded) % 8)).decode("utf-8")
        except (ValueError, UnicodeDecodeError) as exc:
            raise MaskingError(f"cannot unseal serial token: {exc}") from exc

    @staticmethod
    def _split_envelope(token: str, kinds: tuple[str, ...]) -> tuple[str, str, str]:
        """Split ``<kind>-<kid>-<ct>``, checking the kind is one expected."""
        parts = token.strip().lower().split("-", 2)
        if len(parts) != 3 or parts[0] not in kinds or not parts[2]:
            raise MaskingError("not a masked token of the expected type")
        if not _KEY_ID_RE.match(parts[1]):
            raise MaskingError("token carries no key id")
